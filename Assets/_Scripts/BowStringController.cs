...
public class BowStringController : MonoBehaviour
{
    ...
    [SerializeField]
    private float stringSoundThershold = 0.001f;
    ...
    private void HandlePullingString(float midPointLocalZAbs, Vector3 midPointLocalSpace)
    {
        ...
        if (midPointLocalSpace.z < 0 && midPointLocalZAbs < bowStringStretchLimit)
        {
            ...
            strength = Remaap(midPointLocalZAbs, 0, bowStringStretchLimit, 0, 1);
    ...
    {
       if(Mathf.Abs(strength - previousStrength) > stringSoundThershold)
        {
    ...
    private float Remaap(float value, int fromMin, float fromMax, int toMin, int toMax)
    {
